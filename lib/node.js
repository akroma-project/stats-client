...
var WS_SECRET = process.env.WS_SECRET || "asdf";
...
Node.prototype.startSocketConnection = function () {
	if (!this._socket) {
		...
		console.info('wsc', 'Starting socket connection', wsServer);
...
Node.prototype.getStats = function (forced) {
	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true)) {
		...
		try {
			...
			self.setInactive();
			console.error('xx>catch', 'getStats error: ', error);
			return false;
		}
	}
}
...
Node.prototype.getHistory = function (range) {
	...
	async.mapSeries(interv, function (number, callback) {
		...
		function (err, results) {
			...
			const send = results !== undefined && Array.isArray(results) ? results.reverse() : [];
